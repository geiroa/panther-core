...
class Severity {
  static initClass() {
    this.logger = debug('oa:d3:severity');
  }
  ...
  // options:
  static severity_bar(id, data, options) {
    let labelpx, uid;
    options ??= {};
    const self = this;
    ...
    let width = options.width ?? $(id).width();
    ...
    const klass = options.klass ?? null;
    const mouseover = options.mouseover ?? false;

    // Sum the total for each severity
    const sum = d3.sum(data, d => d.total);
    ...
    if (parent != null) {
      ...
      this.logger('all element', uid, id, width, all);
      if (all == null) {
        console.error('no parent: %s', parent);
      }
    }

    // Setup a scale for this bar
    const event_scale = d3.scale.linear().domain([0, sum]).range([0, width]);
    ...
    let rect_start = 0;
    let summary_start = 170;
    const summary_size = (width - summary_start) / (data.length + 1);
    const summary_end = summary_size;

    const summary_scale = d3.scale
      .linear()
      .domain([1, 5])
    ...
    const sev_grouping = svg.append('g');
    sev_grouping.attr('class', 'sev-grouping');

    const sev_bar = sev_grouping.append('g');
    sev_bar.attr('class', 'sev-bar');

    const sev_summary = sev_grouping.append('g');
    sev_summary.attr('class', 'sev-summary');

    for (var sev_count of data) {
      count = sev_count.total;
      var rect_width = event_scale(count);

      this.logger('appending', sev_count);
    ...
    if (mouseover) {
      svg.on('mouseover', function () {
        ...
        d3.selectAll(`.group_${uid}_sev_summary_text`).transition().duration(500).attr('opacity', 1);

        d3.selectAll(`.group_${uid}_sev_summary_rect`).transition().duration(1500).attr('opacity', 0.7);
        ...
        d3.select(`#group_${uid}_rect`).style('stroke', '#333').attr('opacity', 0.6);

        return d3.select(this).selectAll('.sev-bar rect').transition().duration(100).attr('opacity', 0);
      });

      return svg.on('mouseout', function () {
        self.logger('rect mouseout');

        d3.selectAll(`.group_${uid}_sev_summary_text`).transition().duration(500).attr('opacity', 0);

        d3.selectAll(`.group_${uid}_sev_summary_rect`).transition().duration(1500).attr('opacity', 0);
        ...
        d3.select(`#group_${uid}_rect`).style('stroke', '#999').style('opacity', 0.5);

        return d3.select(this).selectAll('.sev-bar rect').transition().duration(500).attr('opacity', 0.8);
      });
    }
  }
}
