...
router.get('/rules/group/:id', function (req, res) {
  res.status(405);
  return res.json({
    name: 'rules/group',
  });
...
// plain get
_.keys(routes).forEach(function (name) {
  debug('setting up route for fetch (api)', name);
  const route = routes[name].path ?? (routes[name].path = `/${name}`);
  return router.get(`${route}`, (...args) => routes[name].handler(name, ...args));
});

// get by id
// _.keys(routes.fetch_id).forEach ( name ) ->
//   debug 'setting up route for fetch by id ', name
//   router.get "/#{name}/:id", ( args... ) ->
