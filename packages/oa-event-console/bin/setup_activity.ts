...
if (opt.help) {
  console.log(' -c --config Event Console config file (including db connection details)' + ' -h --help   This help');
  process.exit(1);
}
...
Mongoose.connect(function () {
  ...
  return Activities.store_Async('provision', 'create', 'system', { name: opt.name })
    .then(function (res) {
      debug('res', res);
      return logger.info('all done [%s]', opt.name);
    })
