...
// logging
let config;
const { logger, debug } = require('oa-logging')('oa:event:setup_severity');
...
if (opt.help) {
  console.log(' -c --config  Event Console config file' + ' -r --reset  Remove current system severities');
  (' -h --help   This help');
  process.exit(1);
}
...
// opt error handling
if (opt.config === true || opt.config === false) {
  opt_error.push('--config argument requires a file');
}

if (opt.reset === !true && opt.reset === !false) {
...
Mongoose.db.once('open', function (cb) {
  ...
  return Severity.find({ system: true })
    .then(function (docs) {
      if (docs.length > 0) {
    ...
    .then(function (results) {
      ...
      return (() => {
        ...
        for (var severity_data of default_severity_data) {
          ...
          result.push(logger.info('Saved %s, result: %s', severity_data.label, results[severity_data.label]._id, ''));
        }
        return result;
      })();
    })
