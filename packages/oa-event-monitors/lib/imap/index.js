  ...
  methods: {
      ...
      async.series(
        {
          connect: function (callback) {
            self.getImapConnection().connect(function (err) {
              if (err) logger.fatal('IMAP connection failed: ' + err + '\n');
              callback(err);
            });
          },
              ...
              [
                function (callback) {
                  ...
                  path.exists(self.getUidFile(), function (exists) {
                    if (exists) {
                      ...
                      callback(null);
                    } else callback(msg_base + ' missing');
                  });
                },
                function (callback) {
                ...
                function (contents, callback) {
                  ...
                  if ((num = parseInt(contents))) {
                    ...
                    self.setTopUid(num);
                    callback(null);
                  } else callback('Failed to parse an integer from ' + self.getUidFile());
                },
              ],
              function (err) {
                /*
    ...
    write_uid: function () {
      ...
      fs.writeFile(self.getUidFile(), '' + self.getTopUid(), function (err) {
        if (err) logger.error('Failed to write IMAP uid (' + self.getTopUid() + ') to file: ' + self.getUidFile());
        else logger.debug('Written IMAP uid (' + self.getTopUid() + ')');
      });
    },
          ...
          async.parallel(
            {
              headers: function (cb) {
                var fetch = self
                  .getImapConnection()
                  .fetch(uid, { request: { body: false, headers: ['from', 'to', 'subject', 'date'] } });
