  ...
  methods: {
    isRecordTerminator: function (record_lines) {
      ...
      if (first_line && first_line.match(/^[0-9]+\/[0-9]+ .*\], Message \[/)) {
        if (last_line && last_line.match(/\]$/)) return true;
        else return false;
      } else {
    ...
    parseRecord: function (record_lines) {
      ...
      if (first_line.match(/Message \[Low\ Idle$/)) first_line += ' ' + record_lines.shift();

      if (
        (matches = first_line.match(/^(\d\d)\/(\d\d) (\d\d):(\d\d) Client \[(\w+)\], Mtype \[(\d+)\], Message \[(.*)/))
      ) {
        ...
        if (record_lines.length == 0) {
          if (message.charAt(message.length - 1) == ']') message = message.slice(0, message.length - 1);
