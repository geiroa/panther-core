...
exports.rules = function (a, obj) {
  ...
  if (a.summary.match(/temporarily disabling Zabbix agent checks on host/i)) {
    a.summary = a.summary.replace(
      /zabbix-server: zabbix_server.log: .* temporarily/,
      'zabbix-server: zabbix_server.log: temporarily'
    );
  }
  ...
  if (
    a.summary.match(
      /svc-spot-prices.php->svc-spot-prices.php.* An error occured. Error: Metal currency is not valid/i
    ) &&
    a.node.match(/gm10027/i)
  ) {
    a.severity = -1; // Discard
  }
  ...
  if (
    a.summary.match(/gm_getrates-frxfeedl.btc.php->gm_frx.inc.php.* failed age verification/i) &&
    a.node.match(/gm26|gm0v/i)
  ) {
    a.severity = -1; // Discard
  }
  ...
  if (
    a.summary.match(
      /gm_rbs-hoca-import.btc.php .* (HOCA import batch script starting|HOCA import batch script finishing|No files were processed|Processing .* file|Could not find any records in file:)/i
    ) &&
    a.node.match(/gm26|gm0v/i)
  ) {
    a.severity = -1; // Discard
  ...
  if (
    a.summary.match(
      /Feed .* for USD and (HKD|CAD)|Cross variance was: .* but threshold was:|has been turned back on. All tests passed|At least one feed for each of the metals and exchange|All feeds for .* and .* are unavailable|Feed .* for .* failed cross variance threshold|Feed .* for .* turned off/i
    ) &&
    a.node.match(/gm26/i)
  ) {
    a.severity = -1; // Discard
  ...
  if (a.summary.match(/AFTR batch script exiting .* no transactions imported/i)) {
    a.summary = a.summary.replace(
      /AFTR batch script exiting .* no transactions imported/,
      'AFTR batch script exiting no transactions imported'
    );
  }
  ...
  if (
    a.summary.match(/SDF Combination Alert \[Classification: Senstive Data\] \[Priority: 2\] .* {PROTO:254}/i) &&
    a.node.match(/hlsnpv01/i)
  ) {
  ...
  if (
    a.summary.match(/Failing Ask price:[0-9]{2}.[0-9]{10} lower than [0-9]{2}.[0-9]{10}/i) &&
    a.node.match(/gm32v/i)
  ) {
    a.summary = a.summary.replace(
      /Failing Ask price:[0-9]{2}.[0-9]{10} lower than [0-9]{2}.[0-9]{10}/,
      'Failing Ask price: X lower than Y'
    );
  }

  if (
    a.summary.match(/Failing Bid price:[0-9]{2}.[0-9]{10} is higher than [0-9]{2}.[0-9]{10}/i) &&
    a.node.match(/gm32v/i)
  ) {
    a.summary = a.summary.replace(
      /Failing Bid price:[0-9]{2}.[0-9]{10} is higher than [0-9]{2}.[0-9]{10}/,
      'Failing Bid price: X is higher than Y'
    );
  }
  ...
  if (
    a.summary.match(
      /cc_id = pc_acc_id#015#012#011#011#011#011and acc_id = pc_acc_id#015#012#011#011#011#011|select count\(\*\) as num_rules from#015#012#011#011#011#011post_rule,#015#012#011#011#011#011post_to_acc,#015#012#011#011#011#011function/i
    ) &&
    a.node.match(/gm42/i)
  ) {
    a.severity = -1; // Discard
  ...
  if (
    a.summary.match(/RepositoryAbstract.php:562 | msg:Invalid argument supplied for foreach/i) &&
    a.node.match(/gm10027/i)
  ) {
    a.severity = -1; // Discard
  }
  ...
  if (
    a.summary.match(
      /unsupported currency and location combo|Got in acle_updateExchRates|Running only holding specific RTE checks/i
    )
  ) {
    a.severity = -1; // Discard
  }
  ...
  if (
    a.summary.match(/\[gm_close-abandoned-hld.btc.php->gm_close-abandoned-hld.btc.php.* (count:|duration:)/i) &&
    a.node.match(/gm26/i)
  ) {
    a.severity = -1; // Discard
  }
  ...
  if (
    a.summary.match(
      /monthly fee batch: failed|monthly fee batch: Cannot directly convert metal to metal|monthly fee batch: unknown combination passed to gm__generateFeeMemo|monthly fee batch: currency to convert should be a metal, not|monthly fee batch: currency .* is invalid|monthly fee batch: invalid storage payment preference|detected another instance of this service - terminating/i
    ) &&
    a.node.match(/gm26/i)
  ) {
  ...
  if (
    a.summary.match(
      /prod2 c4ms-rmws ERROR com.openanswers.core.platform.appsupport.exceptions.AppExceptionHandler ||CAUSE_DEPTH=0||CODE=/
    )
  ) {
  ...
  if (a.summary.match(/warning: .* write queue file: No space left on device/)) {
    a.summary = a.summary.replace(
      /warning: .* write queue file: No space left on device/,
      'warning: write queue file: No space left on device'
    );
  }
  ...
  if (
    a.summary.match(
      /is deprecated. For more information, see|Deprecation notice: must now include|Skipping because of failed dependencies|warning: unable to look up public\/pickup/i
    )
  ) {
    a.severity = -1; // Discard
  }
  ...
  if (
    a.summary.match(
      /No entry is registered for key .*translator|MailChimp logs. Environment: GS. Email: .* Mailchimp code: 1. Mailchimp error: No errors/i
    )
  ) {
    a.severity = -1; // Discard
  }
  ...
  if (
    a.summary.match(
      /Starting holding specific RTE checks|Running both entity and holding specific RTE checks|Starting entity specific RTE checks|Data Cache setting|Max EntCode:|thv\/tnmfv duration|thv\/tnmfv count|initiated delta match/
    )
  ) {
    a.severity = -1; // Discard
  }
  ...
  if (
    a.summary.match(
      /ERROR:  relation .* does not exist at character|ERROR:  current transaction is aborted, commands ignored until end of transaction block|STATEMENT:  SELECT/i
    ) &&
    a.node.match(/hlmnpv02/i)
  ) {
    a.severity = -1; // Discard
  }
  ...
  if (
    a.summary.match(/gm-frxfeedl.* (Starting|Shutting down|Listener started successfully|Listener reached end of life)/)
  ) {
    a.severity = -1; // Discard
  }
  ...
  if (
    a.summary.match(
      /currency feed rate variance too big \[(Morningstar|Interactive) [^0-9]+;[0-9]+;(810|484|356);.*/
    ) &&
    a.node.match(/gm26/i)
  ) {
    a.severity = -1;
  ...
  if (
    a.summary.match(
      /gm_getrates-frxfeedl.btc.php->.*\]* .* currency feed rate variance too big \[(Morningstar|Interactive) [^0-9]+;[0-9]+;840;.*/
    ) &&
    a.node.match(/gm26/i)
  ) {
  ...
  if (
    a.summary.match(
      /gm_getrates-frxfeedl.btc.php->.*\]* .* currency feed rate variance too big \[(Morningstar|Interactive) [^0-9]+;[0-9]+;826;.*/
    ) &&
    a.node.match(/gm26/i)
  ) {
    a.summary = a.summary.replace(/.* currency/, 'gm_getrates-frxfeedl.btc.php-> currency');
  ...
  if (
    a.summary.match(
      /gm_getrates-frxfeedl.btc.php->.*\]* .* currency feed rate variance too big \[(Morningstar|Interactive) [^0-9]+;[0-9]+;978;.*/
    ) &&
    a.node.match(/gm26/i)
  ) {
    a.summary = a.summary.replace(/.* currency/, 'gm_getrates-frxfeedl.btc.php-> currency');
  ...
  if (
    a.summary.match(
      /gm_getrates-frxfeedl.btc.php->.*\]* .* currency feed rate variance too big \[(Morningstar|Interactive) [^0-9]+;[0-9]+;756;.*/
    ) &&
    a.node.match(/gm26/i)
  ) {
    a.summary = a.summary.replace(/.* currency/, 'gm_getrates-frxfeedl.btc.php-> currency');
  ...
  if (
    a.summary.match(
      /gm_getrates-frxfeedl.btc.php->.*\]* .* currency feed rate variance too big \[(Morningstar|Interactive) [^0-9]+;[0-9]+;392;.*/
    ) &&
    a.node.match(/gm26/i)
  ) {
    a.summary = a.summary.replace(/.* currency/, 'gm_getrates-frxfeedl.btc.php-> currency');
  ...
  if (
    a.summary.match(
      /gm_getrates-frxfeedl.btc.php->.*\]* .* currency feed rate variance too big \[(Morningstar|Interactive) [^0-9]+;[0-9]+;036;.*/
    ) &&
    a.node.match(/gm26/i)
  ) {
    a.summary = a.summary.replace(/.* currency/, 'gm_getrates-frxfeedl.btc.php-> currency');
  ...
  if (
    a.summary.match(
      /gm_getrates-frxfeedl.btc.php->.*\]* .* currency feed rate variance too big \[(Morningstar|Interactive) [^0-9]+;[0-9]+;124;.*/
    ) &&
    a.node.match(/gm26/i)
  ) {
    a.summary = a.summary.replace(/.* currency/, 'gm_getrates-frxfeedl.btc.php-> currency');
  ...
  if (
    a.summary.match(
      /gm_getrates-frxfeedl.btc.php->.*\]* .* currency feed rate variance too big \[(Morningstar|Interactive) [^0-9]+;[0-9]+;554;.*/
    ) &&
    a.node.match(/gm26/i)
  ) {
    a.summary = a.summary.replace(/.* currency/, 'gm_getrates-frxfeedl.btc.php-> currency');
  ...
  if (
    a.summary.match(
      /gm_getrates-frxfeedl.btc.php->.*\]* .* currency feed rate variance too big \[(Morningstar|Interactive) [^0-9]+;[0-9]+;344;.*/
    ) &&
    a.node.match(/gm26/i)
  ) {
    a.summary = a.summary.replace(/.* currency/, 'gm_getrates-frxfeedl.btc.php-> currency');
  ...
  if (a.summary.match(/zabbix-server: zabbix_server.log: [\s]*[0-9]+:[0-9]+:[0-9]+\.[0-9]+ item/)) {
    a.summary = a.summary.replace(
      /zabbix-server: zabbix_server.log: [\s]*[0-9]+:[0-9]+:[0-9]+\.[0-9]+/,
      'zabbix_server.log '
    );
  }
  ...
  if (
    a.node.match(/fcr-v-esbint01|fcr-v-esbtest01|[fd]cr-v-.*uat01|[fd]cr-v-esblive01/i) &&
    obj.facility.match(/local3/i)
  ) {
  ...
  if (
    a.summary.match(/ERROR.*org.mule|ERROR.*org.quartz|ERROR.*com.openanswers.*CODE=/) &&
    a.node.match(/lcqcesb|imsuexp/i)
  ) {
    a.severity = 4;
  }

  if (
    a.node.match(/[fd]cr-v-esblive01/i) &&
    a.summary.match(/ERROR [0-9]+-[0-9]+-[0-9]+ [0-9]+:[0-9]+:[0-9]+,[0-9]+/)
  ) {
    a.summary = a.summary.replace(/[0-9]+-[0-9]+-[0-9]+ [0-9]+:[0-9]+:[0-9]+,[0-9]+/, '');
  }

  if (
    a.node.match(/fcr-v-esbint01|fcr-v-esbtest01/i) &&
    a.summary.match(/ERROR [0-9]+-[0-9]+-[0-9]+ [0-9]+:[0-9]+:[0-9]+,[0-9]+/)
  ) {
    a.summary = a.summary.replace(/[0-9]+-[0-9]+-[0-9]+ [0-9]+:[0-9]+:[0-9]+,[0-9]+/, '');
    a.customer = 'C4Test';
  ...
  if (
    a.summary.match(
      /\d+: An account failed to log on. Subject: Security ID: .* Failure Reason: Unknown user name or bad password. .*/
    )
  ) {
    a.severity = -1; // discard
  }
  ...
  if (
    a.summary.match(
      /pam_ck_connector\(gdm:session\): nox11 mode, ignoring PAM_TTY|GConf-WARNING: Got Disconnected from DBus|Invalid legacy unicast query packet|Received response from host .* with invalid source port|Invalid response packet from host|client-conf-x11.c: xcb_connection_has_error/
    )
  ) {
    a.severity = -1; // discard
  }
  ...
  if (
    a.summary.match(
      /.* Unable to contact D-Bus: org\.freedesktop\.DBus\.Error.NotSupported: Unable to autolaunch a dbus-daemon .*/
    )
  ) {
    a.severity = -1; // discard
  }
  ...
  if (a.node.match(/accounts-2.*/i)) {
    ...
    a.node.match(
      /eastwood|burton|hlbkp001|hlmn|hlsfp|hlsq|hlswt|hlvb|hlut|OA_UPS|blmn|blvb|blut|bloapv02|hwm|hwjk|blbkp|plbkp/i
    )
  ) {
    a.customer = 'OA';
    ...
    a.node.match(/gm7v|gm10v|gm11v|gm12v|gm13v|gm14v|gm15v|gm6v|gm9v|gm20v|gm31|gm32|gm33|gm23|gm22|gm2v|gm65/i)
  ) {
    ...
    a.node.match(
      /gm89|gm21|cms3prod.*|cms3db.*|gm1v|gm71|gm26|gm27|gm-mta.*|gm16v|gm100.*|gm40|gm87|gm88|gm90|gm91|gm42|gm17|gm68|gm69|gm28|gm-infra.*|(www|secure).goldmoney.com|gmnfs/i
    )
  ) {
    ...
    a.node.match(
      /oasbs|blsbs|blsbs|hlsbs|hlss|lftpserver|oaxgs20|sbs-scan|blsid|sbs.nhs.uk|app-dev1|gos-test3|owls-pilot/i
    )
  ) {
    a.customer = 'Steria';
   ...
   */
  a.identifier = a.node + ':' + a.severity + ':' + a.summary;
};
