  ...
  async.series(
    {
      ...
      three: function (cb) {
        self.connectToServer(cb);
      },
      four: function (cb) {
        self.registerSignalHandlers(cb);
      },
      five: function (cb) {
        self.loadAgent(cb);
      },
    ...
    function (err, results) {
      ...
      if (err) {
        ...
        if (self.monitor != undefined) {
          self.monitor.sendAlert({
            summary: err,
            node: 'local',
            priority: 4,
            tag: 'PantherAgent',
            identifier: 'local:4:' + err,
          });
        }
        process.exit(1);
      } else {
...
OaMon.prototype.setupNconf = function (callback) {
  ...
  var default_config_file = path.join(oamonhome.getMonitorEtcDir(), monitor_name + '.ini');
  ...
  } else nconf.use('file', { file: config_file });
  ...
  if (nconf.get('version') != undefined) {
    ...
    if (nconf.get('agent:rules').charAt(0) == '/') self.agent_rules_file = nconf.get('agent:rules');
    else self.agent_rules_file = path.join(oamonhome.getMonitorEtcDir(), nconf.get('agent:rules'));
...
OaMon.prototype.connectToServer = function (callback) {
  ...
  if (nconf.get('agent:buffering')) monitor_args['buffering'] = nconf.get('agent:buffering') * 1000;
  ...
  self.monitor.start(function (err) {
    ...
    if (nconf.get('oneshot') == false) self.monitor.sendAlert(self.state.established());
...
OaMon.prototype.loadAgent = function (callback) {
  ...
  try {
    ...
    if (agent_include.Agent == undefined) throw new Error('Missing Agent export in : [' + agent_file_path + ']');
...
OaMon.prototype.startAgent = function (callback) {
   ...
   */
  if (nconf.get('oneshot') == false) this.monitor.sendAlert(self.state.start());
