...
// OAmonServer class definition — extracted from server.js so tests can
// instantiate it without triggering the production auto-start sequence in
// server.js. server.js itself now only wires the production singleton and
// still auto-starts at require time, so every existing entry point
// (server.sh, Dockerfile entrypoint, `yarn start`) is unchanged.
      ...
      connect_to_db: function (cb) {
        ...
        Mongoose.connect(connection_string)
          .then(
            ...
            onRejected => {
              var msg = 'Could not connect to MongoDB on host ' + server_config.DbHostname() + ': ' + onRejected;
              logger.error(msg);
              return cb(onRejected);
            }
          )
          .catch(onrejected => {
